...
const els = {};
const DEFAULT_INPUTS = { S: "3", K: "3", sigma: "20", T: "0.5", r: "2", delta: "0" };
function makeEl(id) {
  const el = {
    id,
    textContent: "",
...
ok("页面加载即按默认参数自动计算（对照表，δ=0 经典 BS）", () => {
  // 与用户给出的参考表一致
  assert.strictEqual(els["t-d1"].textContent, "0.1414");
  assert.strictEqual(els["t-d2"].textContent, "0.0000");
  assert.strictEqual(els["t-Nd1"].textContent, "0.5562");
  assert.strictEqual(els["t-Nd2"].textContent, "0.5000");
  assert.strictEqual(els["t-pd1"].textContent, "0.3950");
  assert.strictEqual(els["t-pd2"].textContent, "0.3989");
  assert.strictEqual(els["t-call"].textContent, "0.1836");
  assert.strictEqual(els["t-put"].textContent, "0.1538");
  assert.strictEqual(els["t-delta"].textContent, "0.5562");
  assert.strictEqual(els["t-gamma"].textContent, "0.9310");
  assert.strictEqual(els["t-vega"].textContent, "0.8379");
  assert.strictEqual(els["t-thetaCall"].textContent, "-0.000540");
  assert.strictEqual(els["t-thetaCallYear"].textContent, "-0.1973");
  assert.strictEqual(els["t-thetaPut"].textContent, "-0.000378");
  assert.strictEqual(els["t-thetaPutYear"].textContent, "-0.1379");
});
...
ok("δ≠0 时按含股息公式计算（S=3,K=3,σ=20%,T=0.5,r=2%,δ=5%）", () => {
  const vals = { S: "3", K: "3", sigma: "20", T: "0.5", r: "2", delta: "5" };
  for (const id of inputIds) { els[id].value = vals[id]; els[id].dispatch("input"); }
